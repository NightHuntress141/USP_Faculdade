...
/**                                                                 **/
/**   <Rebecka Bocci Domingues>                  <15486608>   <02>  **/
/**                                                                 **/
/*********************************************************************/
...
int calcularPenalidade(TIME* time){
    if (time == NULL) return -1; // Verificação de erro

    int penalidade_total = 0;

    for (int i = 0; i < PROBLEMAS; i++) {
        if (time->resolvidos[i]) {
            // Soma o tempo gasto para resolver o problema
            penalidade_total += time->minutos[i];
            // Adiciona a penalidade de 20 minutos para cada tentativa incorreta
...
int calcularProblemasResolvidosDoTime(TIME* time){
	...
	//Endereço for igual a NULL, retorna -1
	if (time == NULL) return -1;

	// Calcula e retorna o numero total de problemas resolvidos pelo respectivo time
	int i;
...
int calcularProblemasNaoResolvidos(MARATONA* mar){
	...
	// Calcula e retorna o número total de problemas que nenhum time ainda conseguiu resolver
	int problemas_nao_resolvidos = 0;

	// Varrer problema por problema de cada time
	for (int i = 0; i < PROBLEMAS; i++) {

        int resolvido_algum_time = false;

		// Inicia a partir do primeiro elemento válido da lista, depois da cabeça
        PONT atual = mar -> cabeca -> prox; 

        // Verifica se algum time resolveu o problema 
        while (atual != NULL) {

            if (atual -> time -> resolvidos[i]) {
                resolvido_algum_time = true;
                break;
            }
        ...
        // Se nenhum time resolveu o problema[i], há uma incrementação no contador
        if (!resolvido_algum_time) {
            problemas_nao_resolvidos++;
        }
    }

    return problemas_nao_resolvidos;
}
...
bool tratarSubmissao(MARATONA* mar, int id, int problema, int tempo, bool acerto){
    ...
    // Localizar o time com o ID fornecido
    while (atual != NULL && atual->time->id != id) {
        atual = atual->prox;
    }

	// Time não encontrado ou problema já resolvido
    if (atual == NULL || atual->time->resolvidos[problema]) {
        return false; 
    }
    ...
    if (acerto) {
        ...
        // Atualizar o status do problema como resolvido
        atual->time->resolvidos[problema] = true;

        // Atualizar o tempo de resolução
        atual->time->minutos[problema] = tempo;

        // Reposicionar o time na lista, se necessário
        PONT aux = atual->ant;
        while (aux != mar->cabeca && calcularProblemasResolvidosDoTime(aux->time) <= calcularProblemasResolvidosDoTime(atual->time)) {
            if (calcularProblemasResolvidosDoTime(aux->time) == calcularProblemasResolvidosDoTime(atual->time) &&
                calcularPenalidade(aux->time) <= calcularPenalidade(atual->time)) {
                break;
            }
            aux = aux->ant;
        }
