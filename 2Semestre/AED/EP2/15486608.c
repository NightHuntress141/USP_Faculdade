...
/**								                                    **/
/**   <Rebecka Bocci Domingues>		  <15486608>   <matutino - 02>  **/
/**								                                    **/
/*********************************************************************/
...
/*Função recursiva auxiliar para buscar uma palavra*/
int buscarPalavraAux(PONT atual, char* palavra, int n, int i) {
    if (palavra[i] && atual->filhos && atual->filhos[(palavra[i] - VALOR_a)]){
        //Varre o caminho para o próximo filho referente a i-esima letra contida na palavra
        PONT proximo = atual->filhos[(palavra[i] - VALOR_a)];
        i++;
        int retorno = buscarPalavraAux(proximo, palavra, n, i);

		//Se chegamos na última letra, então retornamos o contador
        if (i == n) return proximo->contador;
		//Caso contrário, retornamos 0
        else if (retorno == 0) return 0;
...
int buscarPalavra(PONT raiz, char* palavra, int n){
  //No começo, o indice é mandado com valor 0, para ser checado desde o inicio da árvore
  return buscarPalavraAux(raiz, palavra, n, 0);
}

/*Função recursiva auxiliar para inserir uma palavra*/
void inserirAux(PONT atual, char* palavra, int n, int i) {
	//Verificar se não há mais filhos e se a palavra não chegou no fim
    if (!atual->filhos && palavra[i]){
        //Aloca um novo espaço na memória
        atual->filhos = malloc(LETRAS * sizeof(NO**));
        for (int j = 0; j < LETRAS; j++)
        {
            atual->filhos[j] = NULL;
        }
        //Faz as ligações necessárias para o novo filho
        PONT proximo = criarNo();
        atual->filhos[(palavra[i] - VALOR_a)] = proximo;
        i++;
        inserirAux(proximo, palavra, n, i);
        //Na última letra da palavra, adicionamos mais um ao contador
        if (i == n) proximo->contador++;
    }

	//Verificar se ainda há filhos e letras para percorrer
    else if (atual->filhos && palavra[i]){
        //Segue a recursão
        if (atual->filhos[(palavra[i] - VALOR_a)]){
            PONT proximo = atual->filhos[(palavra[i] - VALOR_a)];
            i++;
            inserirAux(proximo, palavra, n, i);
			//Caso seja a última letra, acrescentamos o contador
            if (i == n) proximo->contador++;
        }
        //Cria-se um novo filho para a respectiva letra ainda não existente 
        else{
            PONT proximo = criarNo();
            atual->filhos[(palavra[i] - VALOR_a)] = proximo;
            i++;
...
void inserir(PONT raiz, char* palavra, int n){
	//No começo, o indice é mandado com valor 0, para ser checado desde o inicio da árvore
	inserirAux(raiz, palavra, n, 0); 
}

/*Função recursiva auxiliar para excluir todas as ocorrências de uma palavra*/
int excluirTodasAux(PONT atual, char* palavra, int n, int i){
    if (!atual) return false;
    if (i == n){
        atual->contador = 0;
        return !(atual->filhos);
    }
    int retorno = excluirTodasAux(atual->filhos[palavra[i] - VALOR_a], palavra, n, i + 1);
    if (retorno){
        //Libera espaço na memória
        free(atual->filhos[palavra[i] - VALOR_a]);
        atual->filhos[palavra[i] - VALOR_a] = NULL;
        //Verifica se ainda há filhos
        for (int j = 0; j < LETRAS; j++){
            if (atual->filhos[j]) return false;
        }
		//Libera espaço na memória
        free(atual->filhos);
        atual->filhos = NULL;
...
void excluirTodas(PONT raiz, char* palavra, int n){
	//No começo, o indice é mandado com valor 0, para ser checado desde o inicio da árvore
	excluirTodasAux(raiz, palavra, n, 0); 
}

/*Função recursiva auxiliar para excluir a ocorrência de uma palavra*/
void excluir_auxiliar(PONT atual, char* palavra, int n, int i){
    //Observa se ainda ainda nao percorremos toda a palavra e se ha algo que o segue (um filho), se nao houver retorna falso
    if (atual->filhos && palavra[i]){
		//Verifica se tem o que é buscado
        if (atual->filhos[(palavra[i] - VALOR_a)]){
            PONT proximo = atual->filhos[(palavra[i] - VALOR_a)];
            i++;
			//Chamada recursiva
            excluir_auxiliar(proximo, palavra, n, i);
            if (i == n) proximo->contador--;
        }
    }
}
...
void excluir(PONT raiz, char* palavra, int n){
	//Recebe quantas vezes a palavra aparece
    int contador = buscarPalavra(raiz, palavra, n);

   	//Caso não houver a palavra
	if (contador == 0) return;
	//Caso haja uma palavra
    else if (contador == 1) excluirTodas(raiz, palavra, n);
	//Caso haja mais de uma palavra
    else excluir_auxiliar(raiz, palavra, n, 0);
}
